import {
  authenticate, protectMutation
}
from './auth.js';
import {
  HttpError, json, failure, boundedBody, readJSON
}
from './http.js';
import {
...
function pageOptions(url) {
  ...
  integer(page, 1, 100000, 'Page');
  integer(limit, 1, 50, 'Page size');
  return {
...
async function listItems(env, url, kind, admin) {
  ...
  return json({
    items: rows.results.map(itemView), total: count.total, page, limit
  });
}
...
async function upload(request, env) {
  ...
  return json({
    item: {
      id, filename, mime_type: type, size_bytes: bytes.length, image_url: `/api/admin/media/${id}/file`
    }
  }, 201);
}
      ...
      if (!id && request.method === 'GET') {
        ...
        return json({
          items: rows.results.map(row => ({
            ...row, image_url: `/api/admin/media/${row.id}/file`
          })), total: count.total, page, limit
        });
      }
